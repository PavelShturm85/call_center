...
def callback(manager, message):
    ...
    # Получение активного звонка из группы номеров.
    if 'BridgeEnter' in message.event \
            and phone_number in numbers \
            and ("macro-dialout-trunk" == message.context or "macro-dial-one" == message.context):
    ...
    # До запись статусов и длительности вызова после завершения звонка.
    ...
    # До запись имени аудиофайла и статуса разговора.
